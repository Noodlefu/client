...
public class PlayerDataFactory
{
    ...
    public async Task<CharacterDataFragment?> BuildCharacterData(GameObjectHandler playerRelatedObject, CancellationToken token)
    {
        ...
        try
        {
            ...
            {
                pointerIsZero = true;
                _logger.LogWarning(ex, "Could not check DrawObject for {Object}", playerRelatedObject);
            }
        }
        catch (Exception ex)
        {
        ...
        {
            _logger.LogDebug(ex, "Cancelled creating Character data for {Object}", playerRelatedObject);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to create {Object} data", playerRelatedObject);
        }
    ...
    private async Task<CharacterDataFragment> CreateCharacterData(GameObjectHandler playerRelatedObject, CancellationToken ct)
    {
        ...
        if (objectKind == ObjectKind.Player)
        {
            ...
            _logger.LogDebug("Honorific is now: {Honorific}", playerFragment!.HonorificData);
        ...
        _logger.LogInformation("Building character data for {Object} took {Time}ms", objectKind, TimeSpan.FromTicks(DateTime.UtcNow.Ticks - start.Ticks).TotalMilliseconds);
    ...
    private async Task VerifyPlayerAnimationBones(Dictionary<string, List<ushort>>? boneIndices, CharacterDataFragmentPlayer fragment, CancellationToken ct)
    {
        ...
        foreach (var file in fragment.FileReplacements.Where(f => !f.IsFileSwap && f.GamePaths.First().EndsWith("pap", StringComparison.OrdinalIgnoreCase)).ToList())
        {
            ...
            if (skeletonIndices != null)
            {
                ...
                _logger.LogDebug("Verifying bone indices for {Path}, found {Count} skeletons", file.ResolvedPath, skeletonIndices.Count);
